...
class Comments(Base):
    ...
    objtype = Column(Integer)
    dbid = Column(String(20))
    schname = Column(String(100))
    tabname = Column(String(100))
    colname = Column(String(100))
    memo = Column(String(500))

    index_name = Index('idx', objtype, dbid, schname, tabname, colname)
...
def get_schema_comments(dbid: str):
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(text('''
            SELECT schname, memo FROM comments
            WHERE id in ( SELECT max(id) as id FROM comments
                        WHERE objtype = :objtype and dbid = :dbid
                        GROUP BY schname )
            '''), {'objtype':str(C_SCHEMA), 'dbid':dbid})
            data={}
            for r in result.fetchall():
                data[r[0]]=r[1]
            return data
...
def get_table_comments(dbid: str, schema: str):
    try:
        ...
        with engine.connect() as conn:
            ...
            WHERE id in ( SELECT max(id) as id FROM comments
                        WHERE objtype = :objtype and dbid = :dbid and schname = :schname
                        GROUP BY tabname )
            '''), {'objtype':str(C_TABLE), 'dbid':dbid, 'schname':schema)
...
def get_column_comments(dbid: str, schema: str, table: str):
    try:
        ...
        with engine.connect() as conn:
            ...
            WHERE id in ( SELECT max(id) as id FROM comments
                        WHERE objtype = :objtype and dbid = :dbid and schname = :schname and tabname = :tabname
                        GROUP BY colname )
            '''), {'objtype':str(C_COLUMN), 'dbid':dbid, 'schname':schema, 'tabname':table})
            data={}
            for r in result.fetchall():
                data[r[0]]=r[1]
            return data
...
def set_comments(**args):
    ...
    if type==C_CONN:
        ...
        param.update({'schname': args['schema']})
    elif type==C_TABLE:
        param.update({'schname': args['schema'], 'tabname': args['table']})
    elif type==C_COLUMN:
        param.update({'schname': args['schema'], 'tabname': args['table'], 'colname': args['column']})
    else:
